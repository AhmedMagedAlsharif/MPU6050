...
#define scale_accel16	2048.0				// Scale Accel +/- 16 G
...
void gyro_offset(void){
	...
	if ((offset_gyro[0] != 0) && (offset_gyro[0] != 0) && (offset_gyro[0] != 0))	// If IMU On
		PORTB |= (1 << LED_READY); 													// Ready Led Pin13 On
	else
		gyro_offset();																// Recall function
...
void sample_meters(void){
	...
	for (int i = 0; i < 3; i++){
		accel_correct[i] = lpf_8(accel[i], accel_correct[i], 1);		// Low pass filter x, y, z axis
		//accel_correct[i] = ((accel[i] + accel_correct[i]) >> 1);
	}												
}
...
void drift_correction(void)
{
	...
	for (int i = 0; i < 3; i++){
		accel_vector[i] = accel_correct[i] / scale_accel16;	// Accel vector scaled to G
	}	
...
{		
	...
	double gyro_vector[3];										// Gyro vector scaled												// Incremental time
	...
	gyro_vector[i] = a_gyro[i] * scale_gyro500;		
	}		
